...
IMPORTANT: In a containerized deployment, use `receptor_type` instead of `node_type` to set execution vs. hop. `node_type` is used in RPM-based deployments only.
...
The following diagram describes the Control Plane and Execution Plane node types:

image::1.png[]

The next diagram shows the Execution Plane nodes and their functions:
